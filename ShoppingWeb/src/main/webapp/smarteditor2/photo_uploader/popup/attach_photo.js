    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/shoppingweb/multiplePhotoUpload'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : 'shoppingweb/photoUpload',	//샘플 URL입니다.
 	        sCallback : 'smarteditor2/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	...
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			for (var i=0; i<oFileInfo.length; i++){
				opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML',  ["<img style=\"max-width:640px; \" src='../"+oFileInfo[i].sFileURL+"' />"]);
